    ...
    while true {
      ...
      while let newlineIndex = buffer.firstIndex(of: 0x0A) {
        ...
        if envelope.id == 1 {
          ...
          return try Self.makeSnapshot(from: result)
        }
      }
    }
  }

  static func makeSnapshot(from data: Data) throws -> UsageSnapshot {
    try makeSnapshot(from: JSONDecoder().decode(RateLimitResult.self, from: data))
  }

  private static func makeSnapshot(from result: RateLimitResult) throws -> UsageSnapshot {
    ...
    return UsageSnapshot(
      ...
      fiveHourRemainingPercent: fiveHourWindow.map { max(0, min(100, 100 - $0.usedPercent)) },
      fiveHourResetDate: fiveHourWindow?.resetsAt.map { Date(timeIntervalSince1970: TimeInterval($0)) }
    )
  }
